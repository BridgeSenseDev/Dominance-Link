...
import { removeSectionSymbols } from "./clientUtils.js";
    ...
    for (const msg of splitMessage) {
      ...
      ctx.fillText(
        ...
        Math.round(
          wordX +
            width / 2 -
            ctx.measureText(removeSectionSymbols(header)).width / 2,
        ),
        y - textY[0],
      );
...
export async function renderSkin(
  ...
  const response = await fetch(`https://visage.surgeplay.com/bust/${uuid}`, {
    headers: {
      "User-Agent":
        "Dominance-Link/1.0 (+https://github.com/BridgeSenseDev/Dominance-Link)",
    },
  });
