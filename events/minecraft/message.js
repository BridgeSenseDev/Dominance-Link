...
export default async function execute(client, message, messagePosition) {
  ...
  if (msg.indexOf('Offline Members:') !== -1) {
    ...
    for (let i = messageCache.length - 1; i >= 0; i -= 1) {
      if (messageCache[i].includes('Guild Name:') || messageCache[i].includes('Total Members:') || messageCache[i].includes('Online Members:') || messageCache[i].includes('Offline Members:')) includes += 1;
      if (includes === 4) {
